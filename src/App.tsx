...
import { ArrowDown, ArrowLeft, ArrowRight, Check, Copy, Eye, EyeOff, KeyRound, LockKeyhole, LogOut, Moon, Pencil, Plus, Search, ShieldCheck, Sun, Trash2, X } from 'lucide-react'
...
export default function App() {
  ...
  const [vaultName, setVaultName] = useState('Our shared vault')
  ...
  const [darkMode, setDarkMode] = useState(() => localStorage.getItem('hearth-theme') === 'dark')

  useEffect(() => {
    document.documentElement.dataset.theme = darkMode ? 'dark' : 'light'
    localStorage.setItem('hearth-theme', darkMode ? 'dark' : 'light')
  }, [darkMode])
  ...
  useEffect(() => {
    if (!supabase || !session) { setMembership(null); setVault(null); setPartnerEmails([]); return }
    let active = true
    const client = supabase
    const loadPartner = async (vaultId: string) => {
      const { data } = await client.from('vault_members').select('email').eq('vault_id', vaultId).neq('user_id', session.user.id)
      if (active) setPartnerEmails((data ?? []).map(member => member.email as string).filter(Boolean))
    }
    void (async () => {
      ...
      if (data) {
        ...
        await loadPartner(data.vault_id)
      ...
      } else { setMembership(null); setVault(null); setPartnerEmails([]) }
    })()
    const channel = supabase.channel(`vault-members-${session.user.id}`).on('postgres_changes', { event: '*', schema: 'public', table: 'vault_members' }, payload => {
      if (payload.new && 'vault_id' in payload.new && payload.new.vault_id) void loadPartner(payload.new.vault_id as string)
    }).subscribe()
  ...
  async function makeVault(event: FormEvent) {
    ...
    try {
      ...
      const name = vaultName.trim() || 'Our shared vault'
      const { data: vaultId, error: vError } = await supabase.rpc('create_vault', { p_name: name, p_salt: result.salt, p_wrapped_key: result.wrappedKey })
      if (vError) throw vError
      const v = { id: vaultId as string, owner_id: session.user.id, name }
  ...
  async function renameHousehold(event: FormEvent) {
    event.preventDefault()
    if (!supabase || !vault || !nameDraft.trim()) return
    setBusy(true); setError('')
    try {
      const name = nameDraft.trim()
      const { error: renameError } = await supabase.rpc('rename_vault', { p_vault_id: vault.id, p_name: name })
      if (renameError) throw renameError
      setVault({ ...vault, name }); setVaultName(name); setRenaming(false)
    } catch (e) {
      const detail = messageFor(e)
      setError(detail.includes('rename_vault') || detail.includes('PGRST202') ? 'Household renaming is not enabled in the database yet. Apply the latest Supabase migration, then try again.' : detail)
  ...
  return <div className="app-shell"><aside className="sidebar"><Brand/><div className="workspace"><div className="avatar avatar-pair">{(session.user.email?.[0] ?? 'H').toUpperCase()}</div><div><strong>{vault?.name ?? 'Our shared vault'}</strong><small>Shared space</small></div><ArrowDown size={14}/></div><div className="nav-label">YOUR VAULT</div><button className={`nav-item ${tab === 'all' ? 'active' : ''}`} onClick={() => setTab('all')}><KeyRound size={17}/> All items <span>{entries.length}</span></button><button className={`nav-item ${tab === 'favorites' ? 'active' : ''}`} onClick={() => setTab('favorites')}><span className="star">✳</span> Favorites <span>{favorites.length}</span></button><div className="sidebar-bottom"><div className="security-note"><ShieldCheck size={17}/><div><strong>Encrypted here</strong><small>Only your household can read your data.</small></div></div><div className="profile"><div className="avatar">{(session.user.email?.[0] ?? 'Y').toUpperCase()}</div><div className="profile-name"><strong>{session.user.email}</strong><small>{membership.is_owner ? 'Vault owner' : 'Vault member'}</small></div><button className="icon-button" aria-label="Lock vault" title="Lock vault" onClick={() => { setKey(null); setEntries([]); setSelected(null); setEditing(false); setDraft(empty); setPassphrase('') }}><LockKeyhole size={16}/></button><button className="icon-button" aria-label="Sign out" title="Sign out" onClick={() => void supabase?.auth.signOut()}><LogOut size={16}/></button></div></div></aside>
    <main className="main"><header className="topbar"><div className="crumb">Shared space <span>/</span> <strong>{tab === 'all' ? 'All items' : 'Favorites'}</strong></div><div className="top-actions"><div className="member-stack">{[session.user.email ?? '', ...partnerEmails].filter(Boolean).slice(0, 6).map((email, index) => <div className={`avatar mini ${index > 0 ? 'second' : ''}`} key={email}>{email[0].toUpperCase()}</div>)}{membership.is_owner && partnerEmails.length === 0 && <div className="avatar mini second">+</div>}</div><button className="icon-button theme-toggle" aria-label={darkMode ? 'Use light theme' : 'Use dark theme'} title={darkMode ? 'Use light theme' : 'Use dark theme'} onClick={() => setDarkMode(value => !value)}>{darkMode ? <Sun size={16}/> : <Moon size={16}/>}</button><button className="invite-button" onClick={() => document.getElementById('partner-email')?.focus()}><Plus size={15}/> Invite household member</button></div></header><section className="content"><div className="welcome-row"><div><p className="eyebrow">A LITTLE MORE PEACE OF MIND</p>{renaming ? <form className="household-name-form" onSubmit={renameHousehold}><input autoFocus maxLength={60} value={nameDraft} onChange={e => setNameDraft(e.target.value)} aria-label="Household name"/><button className="icon-button" type="submit" disabled={busy} aria-label="Save household name" title="Save household name"><Check size={17}/></button><button className="icon-button" type="button" onClick={() => setRenaming(false)} aria-label="Cancel household name edit" title="Cancel"><X size={17}/></button></form> : <div className="household-title"><h1>{vault?.name ?? 'Our passwords'}</h1>{membership.is_owner && <button className="icon-button" onClick={() => { setNameDraft(vault?.name ?? ''); setRenaming(true) }} aria-label="Edit household name" title="Edit household name"><Pencil size={15}/></button>}</div>}<p className="muted">A home for the logins your household shares, kept private in one secure place.</p></div><button className="primary add-main" onClick={() => { setSelected(null); setEditing(true); setDraft(empty) }}><Plus size={17}/> Add an item</button></div><div className="toolbar"><div className="search"><Search size={17}/><input placeholder="Search your vault…" value={search} onChange={e => setSearch(e.target.value)}/></div><div className="item-count">{filtered.length} {filtered.length === 1 ? 'item' : 'items'}</div></div>
      <div className="entry-list">{filtered.map((entry, index) => <button className={`entry-card entry-color-${index % 6}`} key={entry.id} onClick={() => { setSelected(entry); setDraft(entry); setEditing(false); setReveal(false) }}><div className="entry-icon">{entry.name.slice(0,1).toUpperCase()}</div><div className="entry-info"><strong>{entry.name}</strong><span>{entry.username || entry.email || 'No username added'}</span></div><span className="entry-kind">Login</span><ArrowRight className="entry-arrow" size={17}/></button>)}</div>
